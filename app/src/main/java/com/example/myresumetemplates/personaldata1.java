...
public class personaldata1 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_personaldata1);
                ...
                if(!validateFullName() | !validateAddress() | !validateContact() | !validateEmail() | !validateSummary() ){
                    ...
                    reference.child(userID).addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            UserPersonalHelper user = new UserPersonalHelper(name.getEditText().getText().toString(),mail.getEditText().getText().toString(),add.getEditText().getText().toString(),phn.getEditText().getText().toString(),summ.getEditText().getText().toString());
                            FirebaseDatabase.getInstance().getReference("users").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child("personaldata1")
                                    .setValue(user).addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {
                                    if (task.isSuccessful())
                                    {
                                        Toast.makeText(personaldata1.this,"Saved Successful",Toast.LENGTH_LONG).show();

                                        openActivity();
                                    }
                                    else{
                                        Toast.makeText(personaldata1.this,task.getException().getMessage(),Toast.LENGTH_LONG).show();
                                    }
                                }
                            });
                        ...
                        public void onCancelled(@NonNull DatabaseError error) {
                            Toast.makeText(personaldata1.this,"Something went wrong",Toast.LENGTH_LONG).show();
        ...
        reference.child(userID).child("personaldata1").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (userPersonalHelper != null){
                    ...
                    name1 = userPersonalHelper.fname1;
                    mail1 = userPersonalHelper.email1;
                    add1 = userPersonalHelper.add1;
                    summ1 = userPersonalHelper.sum1;
                    phn1 = userPersonalHelper.phn1;
    ...
    public void openActivity(){
        Intent intent = new Intent(personaldata1.this, createprofile1.class);

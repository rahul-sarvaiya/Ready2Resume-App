...
public class personaldata3 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
                ...
                if (!validateFullName() | !validateAddress() | !validateContact() | !validateEmail() | !validateSummary()) {
                    ...
                    reference.child(userID).addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            UserPersonalHelper3 user = new UserPersonalHelper3(name.getEditText().getText().toString(), mail.getEditText().getText().toString(), add.getEditText().getText().toString(), phn.getEditText().getText().toString(), summ.getEditText().getText().toString());
        ...
        reference.child(userID).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                UserPersonalHelper3 userPersonalHelper = snapshot.getValue(UserPersonalHelper3.class);

                if (userPersonalHelper != null) {
                    //fname1,email1,add1,phn1,sum1
                    //String name1,mail1,add1,summ1,phn1;

                    name1 = userPersonalHelper.fname3;
                    mail1 = userPersonalHelper.email3;
                    add1 = userPersonalHelper.add3;
                    summ1 = userPersonalHelper.sum3;
                    phn1 = userPersonalHelper.phn3;
